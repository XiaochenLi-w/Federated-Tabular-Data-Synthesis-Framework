...
class View:
    def __init__(self, attr_one_hot: np.array, domain_size_list: np.array):
        ...
        self.domain_size = np.product(
            self.domain_size_list[np.nonzero(self.attr_one_hot)[0]]
        )
    ...
    def calculate_tuple_key(self):
        ...
        self.tuple_key = np.zeros(
            [self.domain_size, self.view_num_attr], dtype=np.uint32
        )
