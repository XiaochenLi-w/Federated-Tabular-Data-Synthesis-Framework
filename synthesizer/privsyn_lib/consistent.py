...
class Consistenter:
        ...
        for key, view in self.views.items():
            ...
            for subset_key, subset_value in subsets_temp.items():
                attributes_intersection = (
                    subset_value.attributes_set & view.attributes_set
                )
    ...
    def consist_views(self):
        ...
        for key, view in self.views.items():
            view.sum = np.sum(view.count)
            view.normalize_count = (
                view.count if view.sum <= 0 else view.count / view.sum
            )
