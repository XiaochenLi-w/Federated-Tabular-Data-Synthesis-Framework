...
import lib.advanced_composition as advanced_composition
...
class DPSyn(Synthesizer):
    ...
    def anonymize(
        self, priv_marginal_sets: Dict, epss: Dict, priv_split_method: Dict
    ) -> Marginals:
        ...
        for set_key, marginals in priv_marginal_sets.items():
            ...
            tmp_num = np.mean(
                [
                    np.sum(marginal.values)
                    for marginal_att, marginal in marginals.items()
                ]
            )
            print(
                "**************** help debug ************** num of records from marginal count before adding noise:",
                tmp_num,
            )
            ...
            noise_type, noise_param = advanced_composition.get_noise(
                eps, self.delta, self.sensitivity, len(marginals)
            )
    ...
    def get_noisy_marginals(self, priv_marginal_config, priv_split_method) -> Marginals:
        ...
        priv_marginal_sets, epss = self.data.generate_marginal_by_config(
            self.data.private_data, priv_marginal_config
        )
        ...
        noisy_marginals = self.anonymize(
            copy.deepcopy(priv_marginal_sets), epss, priv_split_method
        )
        ...
        for select_marginal in noisy_marginals.keys():
            try:
                diff = (
                    noisy_marginals[select_marginal]
                    - priv_marginal_sets["priv_all_two_way"][select_marginal]
                )
            except:
                diff = (
                    noisy_marginals[select_marginal]
                    - priv_marginal_sets["priv_all_one_way"][select_marginal]
                )
            score = diff.sum().sum()
